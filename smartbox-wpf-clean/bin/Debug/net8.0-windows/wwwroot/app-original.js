...
class SmartBoxApp {
    ...
    initializeElements() {
        ...
        console.log('Buttons found:', {
            ...
            openLogs: !!this.openLogsButton,
            analyze: !!this.analyzeButton
        });
    ...
    attachEventListeners() {
        ...
        if (this.captureButton) this.captureButton.addEventListener('click', () => this.capturePhoto());
        if (this.recordButton) this.recordButton.addEventListener('click', () => this.toggleRecording());
        if (this.exportDicomButton) this.exportDicomButton.addEventListener('click', () => this.exportDicom());
        if (this.settingsButton) this.settingsButton.addEventListener('click', () => this.openSettings());
        if (this.debugButton) this.debugButton.addEventListener('click', () => this.toggleDebug());
        if (this.analyzeButton) this.analyzeButton.addEventListener('click', () => this.analyzeCamera());
    ...
    async initWebcam() {
        try {
            ...
            // Init button removed - auto-init on startup
    ...
    openSettings() {
        ...
        iframe.onload = () => {
            // Forward config request to iframe
        ...
        const messageHandler = (e) => {
            if (e.source === iframe.contentWindow) {
                ...
                } else if (e.data.action === 'requestConfig') {
                    // Send current config to settings iframe
                    if (this.config) {
                        iframe.contentWindow.postMessage({
                            action: 'configLoaded',
                            data: this.config
                        ...
                        // Request config from C# host first
                        this.sendToHost('requestConfig', {});
                    }
                } else {
                    // Forward other messages to C# host
                    this.sendToHost(e.data.action, e.data.data);
                }
            }
        };
    ...
    openLogs() {
        ...
        this.openLogsButton.disabled = true;
        this.openLogsButton.innerHTML = '<i class="ms-Icon ms-Icon--CheckMark"></i><span>Opening...</span>';
        setTimeout(() => {
            this.openLogsButton.disabled = false;
    ...
    sendToHost(action, data) {
        ...
        if (window.chrome && window.chrome.webview) {
            ...
            // Show alert for specific actions when not in WebView2
            if (action === 'openSettings') {
                alert('Settings can only be opened when running in the SmartBox application.\n\nTo test: Build and run the app in Visual Studio.');
            } else if (action === 'openLogs') {
                alert('Logs can only be opened when running in the SmartBox application.\n\nTo test: Build and run the app in Visual Studio.');
            }
        }
    }
    ...
    handleHostMessage(event) {
        ...
        switch (message.action) {
            ...
            case 'updateConfig':
                ...
                this.log('Configuration updated from host');
                break;
            ...
            case 'configSaved':
            case 'pacsTestResult':
                // Forward these to settings iframe
                const iframe = document.querySelector('.settings-iframe');
